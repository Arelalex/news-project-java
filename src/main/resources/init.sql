/**
Система Новостной портал.
Пользователь может читать Новости.
Авторизированный Пользователь может создавать свои Новости и редактировать их, а так же оставлять Комментарии к другим новостям.
Модератор может удалять и редактировать Новости и Комментарии.
...
НОВОСТЬ может иметь только одного автора - ПОЛЬЗОВАТЕЛЯ
...
2. Обычный пользователь не может удалять новости и комментарии
3. Обычный пользователь может создавать и редактировать новости и комментарии
...
ALTER TABLE news
   ...
   ALTER TABLE news
       ADD COLUMN reason_rej VARCHAR(128)
